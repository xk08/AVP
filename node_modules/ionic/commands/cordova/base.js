...
class CordovaCommand extends command_1.Command {
    ...
    async preRunChecks(runinfo) {
        ...
        if (this.project.type === 'vue' || this.project.type === 'react') {
            throw new errors_1.FatalException(`Cordova is not supported for ${this.project.type} projects\n\n` +
                `See ${color_1.strong(`https://ionicframework.com/docs/${this.project.type}/overview`)} for detailed information.\n`);
        }
    ...
    async checkForPlatformInstallation(platform, { promptToInstall = false, promptToInstallRefusalMsg = `Cannot run this command for the ${color_1.input(platform)} platform unless it is installed.` } = {}) {
        if (!this.project) {
            throw new errors_1.FatalException('Cannot use Cordova outside a project directory.');
        }
        if (platform) {
